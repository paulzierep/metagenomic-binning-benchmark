...
|---|---|
| Status | 🟢 `running` |
| ⏱ Updated (Europe/Berlin) | `2026-09-24T01:40 CEST` |
| 🏃 Benchmark | no active run |
| 🧠 Model (last turn) | `nemotron-3.5-lightning-free` (watchdog rotates to free models on quota) |
| 📌 Agent note · **52 min old** | Baseline training epoch ~109/200 (~2.4 min/epoch, ETA ~22:35 UTC unless early-stop); next: baseline eval (run_eval.sh) → issue #2 small-dataset test → v1.1.0 fix run
...
| ⚙️ Load · uptime | `0.18 0.36 0.20` · 10 hours, 4 minutes — 32 cores, 62 GiB, no GPU |
| 💾 RAM used/total | `1681/64295 MB` |
| 🔗 Session | `ses_f31799c77ffeTq9gcYgqc4hBhg` |
| 📄 Full state | [PROGRESS.md](PROGRESS.md) · last push `563e77d status: yes — small_test_v2 epoch —/200 loss — acc — model nemot` |
| 📜 Agent transcript | [status/agent-run.log](status/agent-run.log) · `1172425 B` — every run, command & tool result |
