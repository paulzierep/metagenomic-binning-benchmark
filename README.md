...
> 🟢 **Agent status:** `running` · ⏱ `2026-09-24T13:30 CEST` · 🏃 train fix_v11_20260924: epoch 20/200 · loss 3.3221182823181152 · top1 90.60871887207031 · 🧠 `big-pickle` · [status.log](status/status.log)
...
- Figures (issue #12): per-run bar chart `runs/<run>/per_bins.png`
  (per-bin completeness/contamination for both tools) and a comparison scatter
  `results/figures/comp_vs_cont_all-runs.png` (one point per bin, color = run) —
  regenerated by [`scripts/make_per_bin_plots.py`](scripts/make_per_bin_plots.py)
  after every evaluated run.
